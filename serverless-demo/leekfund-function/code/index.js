...
function qryFundInfo(fundCodes) {
  const params = {
    pageIndex: 1,
...
exports.main_handler = async (event, context, callback) => {
  try {
    const { Datas = [] } = await qryFundInfo([320007]);
    console.log(Datas);
    if (Datas.length === 0) {
      Email.sendErrorMail('【失败】LeekFund基金接口检测', '无法获取到数据');
      return [];
    }
    Email.sendSuccessMail('【成功】LeekFund基金接口检测');
    return Datas;
  } catch (err) {
    console.log(err);
    Email.sendErrorMail('【失败】LeekFund基金接口检测', '接口请求不通');
    return null;
  }
};
